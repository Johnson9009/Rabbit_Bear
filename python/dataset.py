...
def minibatch_iterator(data_loader, sample_axis=AxisIndex.FIRST,
    ...
    for features, labels, samples_count in data_loader(shuffle=shuffle):
        ...
        for i in range(minibatches_count):
            mini_features = features.take(sample_indexes[i * minibatch_size:(i + 1) * minibatch_size], axis=sample_axis)
            mini_labels = labels.take(sample_indexes[i * minibatch_size:(i + 1) * minibatch_size], axis=sample_axis)
            yield (mini_features, mini_labels, minibatch_size)
    ...
    if ((drop_tail is False) and (remain_features is not None)):
        assert_that(remain_labels.shape[sample_axis]).is_equal_to(remain_features.shape[sample_axis])
        yield (remain_features, remain_labels, remain_features.shape[sample_axis])
...
class StandardScaler(object):
    ...
    def __init__(self, data_loader, sample_axis=AxisIndex.FIRST, minibatch_size=64):
        ...
        mini_features, _, count = next(minibatch_iter)

        self._sample_axis = sample_axis
        self._mean = np.zeros(get_unit_shape(mini_features, sample_axis))
        self._variance = np.zeros(get_unit_shape(mini_features, sample_axis))
        ...
        while(True):
            try:
                ...
                self._mean += (np.sum((mini_features / self._samples_count), axis=sample_axis, keepdims=True) -
                               (count / self._samples_count) * self._mean)
                # Compute variance iteratively
                if (pre_samples_cnt == 0):
                    self._variance += np.sum(np.square(mini_features - self._mean) / self._samples_count,
                                             axis=sample_axis, keepdims=True)
                else:
                    self._variance += ((np.sum((np.square(mini_features) / self._samples_count), axis=sample_axis, keepdims=True) -
                                        2 * np.sum((self._mean * mini_features / pre_samples_cnt), axis=sample_axis, keepdims=True) +
                                        (count / pre_samples_cnt) * np.square(self._mean) +
                                        np.square(np.sum((mini_features / np.sqrt(self._samples_count * pre_samples_cnt)),
                                                         axis=sample_axis, keepdims=True))) -
